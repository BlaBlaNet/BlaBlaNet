...
function openid_load() {
	GeditLab\Extend\Hook::register('module_loaded','addon/openid/openid.php','openid_module_loaded');
	GeditLab\Extend\Hook::register('reverse_magic_auth','addon/openid/openid.php','openid_reverse_magic_auth');
}

function openid_unload() {
	GeditLab\Extend\Hook::unregister_by_file('addon/openid/openid.php');
}
...
function openid_module_loaded(&$x) {
	if($x['module'] === 'openid') {
		...
		$x['controller'] = new \GeditLab\Module\Openid();
		$x['installed'] = true;
	}
	if($x['module'] === 'id') {
		require_once('addon/openid/Mod_Id.php');
		$x['controller'] = new \GeditLab\Module\Id();
		$x['installed'] = true;
	}
}
