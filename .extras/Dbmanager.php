...
$ACCESS_PWD='HJKGA&HWEkn5#'; #!!!IMPORTANT!!! this is script access password, SET IT if you want to protect you DB from public access
 #DEFAULT db connection settings
 # --- WARNING! --- if you set defaults - it's recommended to set $ACCESS_PWD to protect your db!

$config = dirname(__FILE__).'/.htconfig.php';

// Read config file and punt in database definition
function crm_c($config, &$DBDEF) {
    // Read BlaBlanet configuration
    if (!is_readable($config)) {return false;}
    // Find database properties
    $match=array();$regexp="#\[\s*['\"]db_(host|port|user|pass|data)['\"]\s*\]\s*=\s*['\"]([^'\"]+)['\"]\s*;#";
    ...
    if (isset($keys['db_host'])) {$DBDEF['host'] = $keys['db_host'];}
    if (isset($keys['db_port'])) {$DBDEF['port'] = str_replace(':', '', $keys['db_port']);}
    if (isset($keys['db_data'])) {$DBDEF['db'] = $keys['db_data'];}
    if (isset($keys['db_user'])) {$DBDEF['user'] = $keys['db_user'];}
    if (isset($keys['db_pass'])) {$DBDEF['pwd'] = $keys['db_pass'];}
    return true;
...
<div class="inv">
<a href="/" target="_self"><b>Return to the And Node <?php eo($VERSION)?></b></a>
