...
class Router {
	...
	function __construct(&$a) {
		 ...
		 * GeditLab/Module/Foo.php (object) or file mod/foo.php (procedural)
		...
		$module = \App::$module;
		$modname = "GeditLab\\Module\\" . ucfirst($module);
		...
		if(strlen($module)) {
			...
			if(! (\App::$module_loaded)) {
				try {
					$filename = 'GeditLab/SiteModule/'. ucfirst($module). '.php';
					if(file_exists($filename)) {
						...
						$filename = 'GeditLab/Module/'. ucfirst($module). '.php';
						if(file_exists($filename)) {
							$this->controller = new $modname;
							\App::$module_loaded = true;
						}
