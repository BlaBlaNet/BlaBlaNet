...
class SubModule {
	 ...
	 * This means we'll look for a class Plugins in GeditLab/Module/Admin/Plugins.php
	...
	function __construct($whicharg = 1) {
		...
		$filename = 'GeditLab/Module/' . ucfirst(argv(0)) . '/'. ucfirst(argv($whicharg)) . '.php';
		$modname = '\\GeditLab\\Module\\' . ucfirst(argv(0)) . '\\' . ucfirst(argv($whicharg));
