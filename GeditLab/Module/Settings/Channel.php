...
namespace GeditLab\Module\Settings;
...
class Channel {
		...
		if(($role != $oldrole) || ($role === 'custom')) {
			...
			if($role === 'custom') {
				...
				foreach($global_perms as $k => $v) {
					\GeditLab\Access\PermissionLimits::Set(local_channel(),$k,intval($_POST[$k]));
				}
				...
				$r = q("update channel set channel_allow_cid = '%s', channel_allow_gid = '%s', 
			   	...
			   	$role_permissions = \GeditLab\Access\PermissionRoles::role_perms($_POST['permissions_role']);
				if(! $role_permissions) {
				...
				if($role_permissions['limits']) {
					foreach($role_permissions['limits'] as $k => $v) {
						\GeditLab\Access\PermissionLimits::Set(local_channel(),$k,$v);
					}
				}
	...
	function get() {
		...
		$limits = \GeditLab\Access\PermissionLimits::Get(local_channel());
...
//		$cntunkmail = \App::$user['cntunkmail'];
		...
		$perm_roles = \GeditLab\Access\PermissionRoles::roles();
		...
		$o .= replace_macros($stpl,array(
			...
			'$aclselect' => populate_acl($perm_defaults, false, \GeditLab\Lib\PermissionDescription::fromDescription(t('Use my default audience setting for the type of object published'))),
