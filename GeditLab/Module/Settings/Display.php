...
class Display {
	...
	function post() {
		...
		if($theme == $existing_theme){
			...
			if( ($themeconfigfile = $this->get_theme_config_file($theme)) != null){
				...
				if(class_exists('\\GeditLab\\Theme\\' . ucfirst($theme) . 'Config')) {
					$clsname = '\\GeditLab\\Theme\\' . ucfirst($theme) . 'Config';
	...
	function get() {
		...
		$itemspage = (($itemspage > 0 && $itemspage < 101) ? $itemspage : 50); // default if not set: 50 items
