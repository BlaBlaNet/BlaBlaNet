...
class Magic extends \GeditLab\Web\Controller {
	...
	function init() {
		...
		if(! $x) {
			...
			$j = \GeditLab\Zot\Finger::run((($addr) ? $addr : '[system]@' . $parsed['host']),null);
			if($j['success']) {
				import_xchan($j);
		...
		if(local_channel()) {
			...
			\GeditLab\Zot\Verify::create('auth',$channel['channel_id'],$token,$x[0]['hubloc_url']);
