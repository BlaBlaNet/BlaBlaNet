...
class Connedit extends \GeditLab\Web\Controller {
	...
	function post() {
		...
		if(! $is_self) {
			...
			if($record) {
				\GeditLab\Daemon\Master::Summon(array('Ratenotif','rating',$record));
			}
		}
		...
		if(($_REQUEST['pending']) && intval($orig_record[0]['abook_pending'])) {
			...
			$filled_perms = \GeditLab\Access\Permissions::FilledPerms($abook_my_perms);
		...
		if($new_friend) {
			...
			\GeditLab\Daemon\Master::Summon(array('Onepoll',$contact_id));
	...
	function get() {
		...
		if(argc() == 3) {
			...
			if($cmd === 'update') {
				...
				\GeditLab\Daemon\Master::Summon(array('Poller',$contact_id));
			...
			if($cmd === 'refresh') {
				if($orig_record[0]['xchan_network'] === 'zot') {
					...
					\GeditLab\Daemon\Master::Summon(array('Notifier','permission_update',$contact_id));
				}
				goaway(z_root() . '/connedit/' . $contact_id);
			}
...
//fixme
				...
				$checkinherited = \GeditLab\Access\PermissionLimits::Get(local_channel(),$k);
