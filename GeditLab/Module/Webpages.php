...
class Webpages extends \GeditLab\Web\Controller {
	...
	function get() {
		...
		$x = array(
			...
			'acl' => (($is_owner) ? populate_acl($channel_acl,false, \GeditLab\Lib\PermissionDescription::fromGlobalPermission('view_pages')) : ''),
            ...
            if (!empty($checkedpages)) {
                foreach ($checkedpages as $mid) {
						...
						if($action === 'zipfile') {
								...
								\GeditLab\Lib\ExtendedZip::zipTree($tmp_folderpath, $zip_filepath, \ZipArchive::CREATE);
