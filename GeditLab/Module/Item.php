...
class Item extends \GeditLab\Web\Controller {
	...
	//	 logger('postvars ' . print_r($_REQUEST,true), LOGGER_DATA);
		...
		$view_policy = \GeditLab\Access\PermissionLimits::Get($channel['channel_id'],'view_stream');	
		$comment_policy = \GeditLab\Access\PermissionLimits::Get($channel['channel_id'],'post_comments');
	...
	//		logger('preview: ' . $o, LOGGER_DEBUG);
		...
		if($orig_post) {
			...
			if(! $nopush)
				\GeditLab\Daemon\Master::Summon(array('Notifier', 'edit_post', $post_id));
		...
		if(! $nopush)
			\GeditLab\Daemon\Master::Summon(array('Notifier', $notify_type, $post_id));
