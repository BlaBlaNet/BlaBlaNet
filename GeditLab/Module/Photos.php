...
class Photos extends \GeditLab\Web\Controller {
	...
	function post() {
		...
		$acl = new \GeditLab\Access\AccessList(\App::$data['channel']);
	...
	function get() {
		...
		if( $can_post) {
			...
			$aclselect = (($_is_owner) ? populate_acl($channel_acl,false, \GeditLab\Lib\PermissionDescription::fromGlobalPermission('view_storage')) : '');
	...
	//		logger('mod_photo: link_item' . print_r($link_item,true));
			...
			if($can_post) {
				...
				$aclselect_e = (($_is_owner) ? populate_acl($ph[0], true, \GeditLab\Lib\PermissionDescription::fromGlobalPermission('view_storage')) : '');
				$albums = ((array_key_exists('albums', \App::$data)) ? \App::$data['albums'] : photos_albums_list(\App::$data['channel'],\App::$data['observer']));
