...
class Events extends \GeditLab\Web\Controller {
	...
	function post() {
		...
		$acl = new \GeditLab\Access\AccessList(false);
		...
		if($share)
			\GeditLab\Daemon\Master::Summon(array('Notifier','event',$item_id));
		...
		if($mode == 'view') {
			...
			*/
			...
			$form = replace_macros($tpl,array(
				...
				'$acl' => (($orig_event['event_xchan']) ? '' : populate_acl(((x($orig_event)) ? $orig_event : $perm_defaults), false, \GeditLab\Lib\PermissionDescription::fromGlobalPermission('view_stream'))),
