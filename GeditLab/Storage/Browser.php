	...
	public function htmlActionsPanel(DAV\INode $node, &$output, $path) {
		...
		if ($this->auth->owner_id) {
			...
			if ($channel) {
				...
				$aclselect = ((local_channel() == $this->auth->owner_id) ? populate_acl($channel_acl,false, \GeditLab\Lib\PermissionDescription::fromGlobalPermission('view_storage')) : '');
			}
		}
